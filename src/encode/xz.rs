...
use crate::xz::{crc::CRC32, footer, header, CheckMethod, StreamFlags};
    ...
    W: io::Write,
{
    ...
    {
        let mut digested = util::DigestWrite::new(output, &mut digest);
    ...
    W: io::Write,
{
    ...
    {
        let mut digested = util::DigestWrite::new(&mut footer_buf, &mut digest);
    ...
    W: io::Write,
{
    let (unpadded_size, unpacked_size) = {
        ...
        let mut digest = CRC32.digest();
        {
            let mut digested = util::DigestWrite::new(&mut count_output, &mut digest);
            let header_size = 8;
